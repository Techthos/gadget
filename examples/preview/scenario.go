...
func scenarioMenu(withGallery bool) *gadget.Menu {
	items := []gadget.MenuItem{
		...
		{Tool: "list_orders", Label: "Orders", IconSVG: iconBox,
			...
			Badge:       "read", BadgeVariant: gadget.BadgeInfo},
		{Tool: "show_customer", Args: map[string]any{"id": 1}, Label: "Account detail", IconSVG: iconCard,
		...
		{Tool: "edit_customer", Args: map[string]any{"id": 1}, Label: "Edit Ada", IconSVG: iconPencil,
			Description: "Open the edit form prefilled from the store.",
			Badge:       "write", BadgeVariant: gadget.BadgeWarning},
		{Tool: "choose_shipping", Args: map[string]any{"id": 4471}, Label: "Ship ORD-4471", IconSVG: iconTruck,
			Description: "Pick a carrier from options priced at call time.",
			Badge:       "write", BadgeVariant: gadget.BadgeWarning},
		{Tool: "confirm_delete_customer", Args: map[string]any{"id": 3}, Label: "Delete Alan", IconSVG: iconTrash,
	...
	return &gadget.Menu{
		URI:   "ui://preview/menu",
		Title: "Acme Dispatch",
		Intro: "Pick where to start. Each tile calls a tool, and the host opens that tool's widget.",
		Items: items,
		Brand: appBrand(),
		Theme: appTheme(),
	}
